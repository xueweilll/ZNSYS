	...
	<head>
		...
		var ip='createSql?sql=';
			...
			function GetSql(year,month,month2){
				...
				sql+="from (select distinct xmzmc from ah124 where to_char(bcrq,'yyyy')='"+year+"' and to_char(bcrq,'mm')>='"+month+"' and to_char(bcrq,'mm')<='"+month2+"' or ";
				...
				sql+="left join (select xmzmc,sum(ypsl) as ypsl1 from ah124 where to_char(bcrq,'yyyy')='"+(year-2)+"' and to_char(bcrq,'mm')>='"+month+"' and to_char(bcrq,'mm')<='"+month2+"' group by xmzmc)t2 on t1.xmzmc=t2.xmzmc ";
				sql+="left join (select xmzmc,sum(ypsl) as ypsl2 from ah124 where to_char(bcrq,'yyyy')='"+(year-1)+"' and to_char(bcrq,'mm')>='"+month+"' and to_char(bcrq,'mm')<='"+month2+"' group by xmzmc)t3 on t1.xmzmc=t3.xmzmc ";
				sql+="left join (select xmzmc,sum(ypsl) as ypsl3 from ah124 where to_char(bcrq,'yyyy')='"+year+"' and to_char(bcrq,'mm')>='"+month+"' group by xmzmc)t4 on t1.xmzmc=t4.xmzmc ";
				return sql;
			}
		</script>
	</head>
