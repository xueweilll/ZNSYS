...
<link href="statistic/jquery-easyui/themes/icon.css" rel="stylesheet"
	type="text/css" />
<script src="statistic/jquery-easyui/jquery.easyui.min.js"
	type="text/javascript"></script>
<link href="statistic/jquery-easyui/themes/default/easyui.css"
	rel="stylesheet" type="text/css" />
<script src="statistic/jquery-easyui/locale/easyui-lang-zh_CN.js"
	type="text/javascript"></script>
...
<script>
	...
	function Search() {
		...
		var sql1 = "select distinct xmzmc from ah124 where (to_char(bcrq,'yyyy')='"
				+ year
				+ "' and to_char(bcrq,'mm')>='"
				+ month
				+ "' and to_char(bcrq,'mm')<='" + month2 + "') ";
		sql1 += " or (to_char(bcrq,'yyyy')='" + (year - 1)
				+ "' and to_char(bcrq,'mm')>='" + month
				+ "' and to_char(bcrq,'mm')<='" + month2 + "')";
		sql1 += " or (to_char(bcrq,'yyyy')='" + (year - 2)
				+ "' and to_char(bcrq,'mm')>='" + month
				+ "' and to_char(bcrq,'mm')<='" + month2 + "')";
					...
					success : function(data) {
						...
						for (var i = 0; i < data.length; i++) {
							...
							$.ajax({
								async : false,
								dataType : 'jsonp',
								url : encodeURI(ip + arrsql[i]),
								jsonp : 'jsonpCallBack',
								success : function(data2) {
									console.info(data2);
									var arr = new Array("jcxm", "zxm",
											"ypsl1", "ypsl2", "ypsl3");
	...
	function GetArrsql(data, year, month, month2) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			arr[i] += " from (select distinct jcxm,zxm from ah124 where xmzmc='"
					+ data[i]
					+ "' and (to_char(bcrq,'yyyy')='"
					+ year
					+ "' and to_char(bcrq,'mm')>='"
					+ month
					+ "' and to_char(bcrq,'mm')<='" + month2 + "'";
			arr[i] += " or to_char(bcrq,'yyyy')='" + (year - 1)
					+ "' and to_char(bcrq,'mm')>='" + month
					+ "' and to_char(bcrq,'mm')<='" + month2
					+ "' or to_char(bcrq,'yyyy')='" + (year - 2)
					+ "' and to_char(bcrq,'mm')>='" + month
					+ "' and to_char(bcrq,'mm')<='" + month2
					+ "')) t1 left join ";
			arr[i] += " (select jcxm,zxm,sum(ypsl) as ypsl1 from ah124 where xmzmc='"
					+ data[i]
					+ "' and to_char(bcrq,'yyyy')='"
					+ (year - 2)
					+ "' and to_char(bcrq,'mm')>='"
					...
					+ "' and to_char(bcrq,'mm')<='"
					+ month2
					+ "' group by jcxm,zxm) t2 ";
			arr[i] += " on t1.jcxm=t2.jcxm and t1.zxm=t2.zxm left join ";
			arr[i] += " (select jcxm,zxm,sum(ypsl) as ypsl2 from ah124 where xmzmc='"
					+ data[i]
					+ "' and to_char(bcrq,'yyyy')='"
					+ (year - 1)
					+ "' and to_char(bcrq,'mm')>='"
					...
					+ "' and to_char(bcrq,'mm')<='"
					+ month2
					+ "' group by jcxm,zxm) t3 ";
			arr[i] += " on t1.jcxm=t3.jcxm and t1.zxm=t3.zxm left join ";
			arr[i] += " (select jcxm,zxm,sum(ypsl) as ypsl3 from ah124 where xmzmc='"
					+ data[i]
					+ "' and to_char(bcrq,'yyyy')='"
					+ year
					+ "' and to_char(bcrq,'mm')>='"
...
<body class="easyui-layout">
	<div class="easyui-panel" data-options="region:'north'"
		style="height: 30px;" border="false">
		<table>
			<tr>
				<td>报出日期</td>
				<td><input id="year" class="easyui-combobox" /></td>
				<td>年</td>
				<td><select id="month" class="easyui-combobox"
					style="width:50px;">
				...
				<td>月</td>
				<td><select id="month2" class="easyui-combobox"
					style="width:50px;">
						<option value="1">01</option>
						<option value="2">02</option>
						<option value="3">03</option>
