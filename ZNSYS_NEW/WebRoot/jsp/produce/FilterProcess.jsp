...
<script>
	var jcxm='',area='',well='',ph='';
	function searchForm(){
    	//alert('aa');
    	jcxm = myEncoder($('#program').combobox('getValue'));
    	//csname = myEncoder($('#cs').combobox('getValue'));
    	area = myEncoder($('#area').combobox('getValue'));
    	well = myEncoder($('#well').combobox('getValue'));
    	ph = myEncoder($('#ph').combobox('getValue'));
    	$('#grid').datagrid({url : 'JsonProcess?jcxm='+jcxm+'&area='+area+'&well=&ph='+ph});
    }
	var FlowContent;
	$(function() {
		...
		$('#grid').datagrid({
			title : '样品列表',
			url : 'JsonProcess?jcxm='+jcxm+'&area='+area+'&well=&ph='+ph,
			fit : true,
			nowrap : false,
			rownumbers : true,
			striped : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					if(value != 'n')
						return "<font color='#000'>"+value+"</font>"
				}
			}, {
				title : '岗位接样',
				field : 'JYR',
		...
		$("#program").combobox(
				{
					...
					onChange : function(newValue, oldValue) {
						if (newValue != null) {
							var thisKey = myEncoder(newValue); //搜索词
							var urlStr = "searchcs?searchProgram="
									+ thisKey;
							//alert(thisKey);		
							//$("#cs").combobox("reload", urlStr);
							urlStr = "searchWell?searchWell=" + thisKey;
							$("#well").combobox("reload", urlStr);
							urlStr = "searchArea?searchArea=" + thisKey;
							$("#area").combobox("reload", urlStr);
							urlStr = "searchPh?searchPh=" + thisKey;
							$("#ph").combobox("reload", urlStr);
						} 
						/*
						else {
							var thisKey = "";
							var urlStr = "searchProgram?searchProgram="
									+ thisKey;
							$("#program").combobox("reload", urlStr);
						}*/
					}
					/*,
					onDblClickRow : function(rowindex, rowdata) {
					...
					onSelect : function(rec) {
						var url = 'JsonProcessByJcxm?jcxm='
								+ myEncoder(rec.jcxm);
						$('#grid').datagrid({
							url : url
						});
	...
	function FlowOpen(ypph, jcxm) {
		document.getElementById('ListProduce').contentWindow
				.Loading(ypph, jcxm);
	}
...
<body class="easyui-layout">
	...
	<div id="panel"
		data-options="border:false,region:'north'"  style="height:35px">
		<table
			style="width:100%;height:auto;padding:0px;font-size:small;font-weight:bolder"
			align="center">
			<style>
.input {
	margin-right: 20px;
			...
			<tr>
				<td>项目：<input id="program" class="input" /> <!--参数：<input class="easyui-combobox" id="cs"
					class="input" /> -->地区：<input id="area" class="input" /> 井号：<input
					id="well" class="input" /> 批号：<input id="ph" class="input" /> <a href="javascript:void(0)" class="easyui-linkbutton" onclick="searchForm()">查询</a>
				</td>
			</tr>
	...
	<div id="FlowContent">
		<iframe width="100%" height="100%" src="jsp/produce/ListProduce.jsp"
			id="ListProduce" name="ListProduce" scrolling="no" frameborder="0"></iframe>
	</div>
	<div id="preview" class="show_area">
		<span id="spans"></span>
	</div>
</body>
</html>
