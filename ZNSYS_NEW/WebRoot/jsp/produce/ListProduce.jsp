	...
	var datawin;
		...
		function LoadFlow(){
			...
			$.ajax({
				...
				url: "ZGC?ypph="+myEncoder(ypph)+"&jcxm="+myEncoder(jcxm)+"&csname="+myEncoder(csname),
				dateType: "json",			
		...
		//获取流程图中的生产过程
		function getProduce(SCGC,CSNAME){
			scgc = SCGC;
			$.ajax({
				type: "POST",
				url: "JsonDatagridBySCGC?ypph="+myEncoder(ypph)+"&jcxm="+myEncoder(jcxm)+"&scgc="+myEncoder(scgc) + "&csname="+myEncoder(CSNAME),
				dateType: "json",			
				success: function(data,status){ 
					...
					if(data == ""){
						...
						alert("不存在查询条件！");
						return;
					}else if(data.indexOf("error_4") != -1){
						var s= data.substring(data.indexOf("error_4")+7);
						alert("查询条件中未给字段"+ s +"赋值!");
						return;
					}
					var str = data.split("~");
					try{
