...
<script>
			...
			jcmxCombo = $("#jcxm").combobox({
				...
				textField:'text',
         		onSelect: function (rec){
         			...
         			var url='JsonSCGCbyJCXM?jcxm='+myEncoder(rec.text);  
         			nodegrid.datagrid({url:url});
         			nodegrid.datagrid("reload");
         		}   
			});
		...
		function addNode(){
			...
			if (row == null)
				...
				addform=addwin.find('form');
				document.getElementById('flow').contentWindow.addNode(row.id.scgc,row.id.csname);
			}
		}
...
<body class="easyui-layout" id="LayoutBody">
	...
	<!-- 参数名弹出框 -->
<!-- 	<div id="addcsname" style="width:400px;height:150px;">
		<div id="cs_content" style="padding:10px 10px 10px 20px;">
		<br/>
		<form>
			<table>
				<tr>
					<td align="right"  width="100"><div align="right"><sup>*</sup>新参数名称:</div></td>
					<td>
						<input type="text" id="txtCsname" name="title" style="width:200px;box-shadow:0px 0px 3px rgba(0,0,0,0.3); "/>
					</td>
				</tr>						
			</table>
			<br/>
			<p align="center">
				<a href="#" class="easyui-linkbutton" icon="icon-save" onClick="addCsname();" style="margin-right:15px;">确定</a>
				<a href="#" class="easyui-linkbutton" icon="icon-cancel" onClick="$('#addcsname').window('close')">取消新建</a>
			</p>
	  	</form>
		</div>
	</div> -->
