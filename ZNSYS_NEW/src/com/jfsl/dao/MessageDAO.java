...
public class MessageDAO extends DAOImp
{
	...
	public List<Message> Find(int messageid)
	{
		...
		Query query=this.getOracleCurrentSession().createQuery("select m from Message as m where m.messageid=:messageid");
	...
	public List<Message> ListUsed()
	{
		...
		/**query*/
		Query query = this.getOracleCurrentSession().createQuery("select m from Message as m where m.dateline >= :nowtime");
	...
	public List<Message> QueryTime(String begintime,String endtime)
	{
		...
		Query query = this.getOracleCurrentSession().createQuery("select m from Message as m where m.publishtime > :begintime and m.publishtime <:endtime");
	...
	{
		Query query = this.getOracleCurrentSession().createQuery("select m from Message as m where m.author = :author");
