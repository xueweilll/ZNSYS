...
public class DepartmentDAO extends DAOImp {
	...
	{
		String sqlstr="select id,name,showorder,belongin from V_SYS_DEPARTMENTS ";
	...
	{
		Department dep=(Department)this.getOracleCurrentSession().get(Department.class, id);
		return dep;
	}
	...
	public String GenerateID(String parentid)
	{
		...
		Query query=this.getOracleCurrentSession().createQuery(strHql);
	...
	{
		String sqlstr="select id from SYS_DEPARTMENTS ";
		sqlstr=sqlstr+"where substr(id,len(id)-3)='"+id.substring(0,id.length()-3)+"' ";
