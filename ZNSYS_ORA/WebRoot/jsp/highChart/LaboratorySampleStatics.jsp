	...
	<head>
		...
		<script>
			...
			function GetSql(year,month){
				...
				sql+="from syzx_ah124 where to_char(bcrq,'yyyy')='"+year+"' and to_char(bcrq,'mm')<='"+month+"' group by ejdw, xmzmc order by ejdw) f1 full outer join (select ejdw,xmzmc,ypsl2 from (select ejdw, ";
				sql+="xmzmc,sum(ypsl)as ypsl2 from syzx_ah124 where to_char(bcrq,'yyyy')='"+year+"' and to_char(bcrq,'mm')='"+month+"' group by ejdw, xmzmc order by ejdw) ";
