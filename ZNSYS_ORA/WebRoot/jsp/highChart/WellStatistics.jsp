...
<html>
	...
	<script>
	ip='http://10.72.21.233:8080/ZNSYS/createSql?sql=';
			$(function(){	    		
	    		$('#dq').combobox({
	    			textField:'dq',
	    			valueField:'dq',
	    		...
	    		$.ajax({
	    			async:false,
	    			dataType:'jsonp',
	    			url:ip+"select distinct dq from syzx_ajhf03 where dq is not null",
	    			jsonp: 'jsonpCallBack',
	    		...
	    		$('#start').datebox({
	    			onSelect:function(date){	
	    				...
	    				var ypph="select distinct dq from syzx_ajhf03 where dq is not null";
	    				if(time1!=""){
			...
			function GetSql1(dq,time1,time2){
	    		var sql="select jh,sum(ypsl) as ypsl,sum(bgzypsl) as bgzypsl from syzx_ajhf03 where dq='"+dq+"'";	    		
    			sql+=" group by jh";
    			
    			return myEncoder(sql);
	    	}
//			function GetSql2(dq,time1,time2){
//				var sql="select jh,count(distinct ypph)as ps from syzx_ajhf03 where dq='"+dq+"' and dq is not null ";				
//  			sql+=" group by jh";
//  			return sql;
//			}
	    	function Search(){
