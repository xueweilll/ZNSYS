...
<html>
	...
	<head>
		...
		var ip='http://10.72.21.233:8080/ZNSYS/createSql?sql=';
			$(function(){								
	    		var year="select distinct substr(to_char(fxrq,'yyyy-mm-dd'),1,4)as fxrq from syzx_ajhf03 where fxrq is not null order by fxrq";
	    		$('#fxrq').combobox({
	    			width:100,
	    			editable:false,
			...
			function GetSql(time1,time2){
	    		var sql="select ypph,sum(bgzypsl) as bgzypsl,sum(ypsl) as ypsl from syzx_ajhf03 where 1=1";
	    		if(time1!=""){
	    	...
	    	function Search(){	    		
	    		...
	    		switch(fxrq){
	    			case "year":	    			
		    			sql1="select fxrq,sum(ypsl) as ypsl, sum(bgzypsl) as bgzypsl from (select substr(to_char(fxrq,'yyyy-mm-dd'),0,4)as fxrq,ypsl,bgzypsl from syzx_ajhf03";
		    			...
		    			sql2+=" from syzx_ajhf03 where fxrq is not null) group by fxrq order by fxrq";
		    			xtext1="年份";		    			
	    			break;
	    			case "quarter":
		    			if(year==""){
		    			...
		    			sql1+=" ypsl,bgzypsl,substr(to_char(fxrq,'yyyy-mm-dd'),6,2) as month from syzx_ajhf03 where fxrq is not null and";
		    			...
		    			sql2+=" (select distinct ypph,substr(to_char(fxrq,'yyyy-mm-dd'),6,2)as month from syzx_ajhf03";
	    			...
	    			case "month":
		    			...
		    			sql1+=" substr(to_char(fxrq,'yyyy-mm-dd'),6,2) as month from syzx_ajhf03";
		    			...
		    			sql2+="substr(to_char(fxrq,'yyyy-mm-dd'),6,2) as month from syzx_ajhf03 where fxrq is not null and substr";
	    			...
	    			case "day":
		    			...
		    			sql1="select to_char(fxrq,'yyyy-mm-dd') as fxrq,sum(ypsl) as ypsl,sum(bgzypsl) as bgzypsl from syzx_ajhf03"
						...
						sql2="select to_char(fxrq,'yyyy-mm-dd') as fxrq,count(distinct ypph) as ps from syzx_ajhf03 where fxrq is not null  ";
						if(time1!=""){
