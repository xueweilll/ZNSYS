...
public class ZGCDAO extends DAOImp
{
	...
	{	
		String sql = "select SCGC from SYZX_SY_ZGC where YPPH='"+ypph+"' and JCXM = '"+jcxm+"' and ( CSNAME='"+csname+"' or CSNAME='" + jcxm + "')";
	...
	{	
		String sql = "select SCGC from SYZX_SY_ZGC where YPPH='"+ypph+"' and JCXM = '"+jcxm+"'";
		//System.out.println("Oracle sql £º"+ sql);
		Query query=this.getOracleCurrentSession().createSQLQuery(sql);
		List ls=query.list();
		return ls;
