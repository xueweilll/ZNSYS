...
public class PushDAO extends DAOImp
{
	...
	{
		Query query=this.getCurrentSession().createQuery("select p from Push as p where p.id=:id");
	...
	public List<Push> ListUsed()
	{
		...
		/**query*/
		Query query = this.getCurrentSession().createQuery("select p from Push as p where p.publishtime <= :nowtime and p.deadline >= :nowtime");
